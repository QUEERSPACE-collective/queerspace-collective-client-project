...
router.get('/:id', (req, res) => {
  if (req.user.userType > 3) {
    console.log('allusers/:id GET req.params.id is', req.params.id);
    const id = req.params.id;
    const sqlText = `SELECT * FROM "user" WHERE "user"."id" = $1;`;
    const sqlParams = [id]; // $1 = req.params.id
    console.log('and the sqlParams are', sqlParams);
    ...
    console.log('allusers/:id GET req.params.id is', req.params.id);
    const id = req.params.id;
    const sqlText = `SELECT * FROM "user" WHERE id = $1 ORDER BY id ASC;`;
    const sqlParams = [id]; // $1 = req.params.id
    console.log('and sqlParams are ', sqlParams);
...
router.put('/:id', (req, res) => {
  if (req.user.userType == 5) {
    ...
    console.log('sqlText, and sqlParams', sqlText, sqlParams);
    pool.query(sqlText, sqlParams)
      .then((dbRes) => {
        res.sendStatus(200);
      })
